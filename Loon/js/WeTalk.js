...
@Author: TG@ZenMoFiShi (QX原版) → 适配Loon by Assistant

使用说明：
1. 将本脚本保存到 Loon 可访问的路径（如 iCloud 或本地文件）
2. 在 Loon 配置中添加：
   [Script]
   # 抓包更新账号 (http-request)
   http-request ^https:\/\/api\.wetalkapp\.com\/app\/queryBalanceAndBonus script-path=WeTalk_Loon.js

   # 定时任务 (cron)
   cron "20 8,20 * * *" script-path=WeTalk_Loon.js

3. 手机开启 MITM 并信任证书，首次打开 WeTalk 触发抓包即自动录入账号
*/
...
async function runAccount(acc, index, total) {
  ...
  async function doVideoLoop(count) {
    for (let i = 1; i <= count; i++) {
      ...
      try {
        const resp = await fetchApi('videoBonus');
  ...
  try {
    ...
    // 最终余额
    resp = await fetchApi('queryBalanceAndBonus');
    d = JSON.parse(resp.body);
...
if (typeof $request !== 'undefined' && $request && $request.url) {
  ...
  (async () => {
    ...
    for (let idx = 0; idx < ids.length; idx++) {
      const id = ids[idx];
      const text = await runAccount(store.accounts[id], idx, total);
      results.push(text);
